...
@author: KenziD
email:Rob10085@gmail.com

运行要求：
1. 安装处理工具包(pip install lxml pyExcelerator xlrd)。
2. 本程序路径最好不要有中文。
...
class XslWriter(object):
    ...
    def creat(self):
        ...
        self.ws.write(0,2,u'YearMonthDate')
        self.ws.write(0,3,u'HourMinSec')
        self.ws.write(0,4,u'Retweet')
        self.ws.write(0,5,u'Reply')
        self.ws.write(0,6,u'Title')
        self.ws.write(0,7,u'Link')
        self.ws.write(0,8,u'Text')
        self.ws.write(0,9,u'MessageId')
        self.ws.write(0,10,u'Target')
...
def readXls():
    ...
    # {"大东海B":ZH200613}
    stockNameId_dict = dict(zip(stockName_list,stockid_list))
    ...
    def parse_process(self,stockId,stockName):
        while True:
            ...
            if news_json is None:
                logger.info("fail to get %s json" % stockName)
                if self.outsideRetry>0:
                    logger.info("outsideRetry %s to get %s json" % (self.outsideRetry,stockName))
                if (self.outsideRetry == 4):
                    self.outsideRetry = 0
                    logger.info("########################END %s####################" %stockName)
            ...
            if max_page == 0:
                logger.info("##############max_page==0##########END %s####################" %stockName)
                self.xslWriter.close(stockId)
                break
            for pageIndex in range(1,max_page+1):
                ...
                while True:
                    if(retry>0):
                        logger.info("retry %s to get json %s page %s "%(retry,stockName,pageIndex))
                    if (retry == 50):
                        retry = 0
                        logger.info("########################END %s####################" %stockName)
                        self.xslWriter.close(stockId)
                    ...
                    if news_json is None:
                        logger.info("fail to get json %s page %s" % (stockName,pageIndex))
                        retry = retry+1
                        continue
                    decode_news_json = json.loads(news_json)
                    logger.info("success to get json %s page %s" %(stockName,pageIndex))
                    for item in decode_news_json['list']:
                        ...
                        YMD = formateTime[0:10]
                        # logger.debug(YMD)
                        TIME = formateTime[10:19]
                        # logger.debug(TIME)
                        text = item['text']
                        if text is None:
                            text = "None"
    ...
    while True:
        ...
        for filename in fn:
            if os.stat('NewsLinkText/%s'%filename).st_size/1024.0<7:
                logger.info( ('empty file is %s'%filename))
